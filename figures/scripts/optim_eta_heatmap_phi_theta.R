...
# Load data
df_eta_optim <- readRDS("results/optim_eta_2_original.rds")
...
plt_x2_slice <- ggplot(df_x2, aes(x = x1, y = elpd)) +
  ...
  labs(y = "ELPD",
       ...
       color = expression(log[10](eta)),
       subtitle = expression(bar(x)[2] == 1.2))
...
ggsave("figures/optim_eta_heatmap_phi_theta.png", width = 16, height = 6, dpi = 300, units = "cm")
