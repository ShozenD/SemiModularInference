...
# Load data
df_eta_optim <- readRDS("results/optim_eta_joint_original.rds")
...
plt_heatmap <- ggplot(df_eta_optim, aes(x1, x2)) +
  ...
  labs(x = expression(bar(x)[1]),
       ...
       subtitle = bquote("Optimal" ~ eta ~ "on" ~ Y[1] ~ "and" ~ Y[2]))
...
ggsave("figures/optim_eta_heatmap_joint.png", width = 16, height = 6, dpi = 300, units = "cm")
