...
# Load data
df_eta_optim_phi <- readRDS("results/eta_optim_phi_original.rds")

# Slice along x1
x1_cut <- -0.25

df_x1 <- df_eta_optim_phi %>%
...
plt_x2_slice <- ggplot(df_x2, aes(x = x1, y = elpd)) +
  ...
  labs(y = "ELPD",
       ...
       color = expression(log[10](eta)),
       subtitle = expression(bar(x)[2] == 1.125))
...
plt_heatmap_phi <- ggplot(df_eta_optim_phi, aes(x1, x2)) +
  ...
  labs(x = expression(bar(x)[1]),
       ...
       subtitle = bquote("Optimal" ~ eta ~ "for" ~ varphi ~ "on" ~ Y[1]))

plt_heatmap_phi + plt_x1_slice + plt_x2_slice +
...
ggsave("figures/optim_eta_heatmap_phi.png", width = 16, height = 6, dpi = 300, units = "cm")
